...
The architecture has to be decided before `hl.h` is included, since `hl.h` defines the architecture
macros back again, and decided a second time by the build, which has to know which backend to hand the
compiler. `hxs_arch.h` is that one decision: `hxs.c` includes it, and the build preprocesses
`hxs_arch_probe.c` beside it with the compiler and flags it is about to use, so a cross build is
answered for what it targets rather than for the machine doing the targeting.

**hashlink's jit is x86 only, so arm64 has one of this library's own.** `src/jit.c` upstream is an x86
and x86-64 instruction encoder with no other backend behind it, and its own guard is `#ifdef __arm__`,
which 64 bit ARM does not define, so left to itself it compiles and emits instructions the processor
cannot run. `src/hxscript/hl/native/arm64/` is the counterpart: it answers the same seven functions
`module.c` reaches a jit through, so `code.c` and `module.c` are shared between the two architectures
and only the encoder differs. Anywhere that is neither x86-64 nor arm64 has no loader at all and
interprets.
